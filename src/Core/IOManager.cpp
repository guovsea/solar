...
int IOManager::addEvent(int fd, Event event, std::function<void()> cb) {
    ...
    if (fd_ctx->events & event) {
        SOLAR_LOG_ERROR(g_logger) << "addEvent assert fd=" << fd << " event=" << event
    ...
    if (rt) {
        SOLAR_LOG_ERROR(g_logger) << "epoll_ctl(" << m_epfd << ", " << op << "," << fd << "," << epevent.events
        ...
        for (int i = 0; i < rt; ++i) {
            ...
            if (rt2) {
                SOLAR_LOG_ERROR(g_logger)
                        << "epoll_ctl(" << m_epfd << ", " << op << "," << fd_ctx->fd << "," << event.events
