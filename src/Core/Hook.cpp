...
struct _HookIniter {
  _HookIniter() {
    ...
    g_tcp_connect_timeout->addListener(
        0x55585, [](const int &old_value, const int &new_value) {
          SOLAR_LOG_INFO(g_logger) << "tcp connect timeout changed: from "
...
retry:
  ...
  if (n == -1 && errno == EAGAIN) {
    ...
    if (rt) {
      ...
      SOLAR_LOG_DEBUG(g_logger)
          << "do_io<" << hook_fun_name << "> yield to hold";
      Fiber::YeildToHold();
...
int connect(int sockfd, const struct sockaddr *addr, socklen_t addrlen) {
  return solar::connect_with_timeout(sockfd, addr, addrlen,
                                     solar::s_connect_timeout);
}

int accept(int s, struct sockaddr *addr, socklen_t *addrlen) {
  int fd = solar::do_io(s, accept_f, "accept", solar::IOManager::READ,
...
ssize_t read(int fd, void *buf, size_t count) {
  return solar::do_io(fd, read_f, "read", solar::IOManager::READ, SO_RCVTIMEO,
                      buf, count);
}
size_t readv(int fd, const struct iovec *iov, int iovcnt) {
  return solar::do_io(fd, readv_f, "readv", solar::IOManager::READ, SO_RCVTIMEO,
                      iov, iovcnt);
}

ssize_t recv(int sockfd, void *buf, size_t len, int flags) {
  return solar::do_io(sockfd, recv_f, "recv", solar::IOManager::READ,
                      SO_RCVTIMEO, buf, len, flags);
}

ssize_t recvfrom(int sockfd, void *buf, size_t len, int flags,
                 struct sockaddr *src_addr, socklen_t *addrlen) {
  return solar::do_io(sockfd, recvfrom_f, "recvfrom", solar::IOManager::READ,
                      SO_RCVTIMEO, buf, len, flags, src_addr, addrlen);
}

ssize_t recvmsg(int sockfd, struct msghdr *msg, int flags) {
  return solar::do_io(sockfd, recvmsg_f, "recvmsg", solar::IOManager::READ,
                      SO_RCVTIMEO, msg, flags);
}

ssize_t write(int fd, const void *buf, size_t count) {
  return solar::do_io(fd, write_f, "write", solar::IOManager::WRITE,
                      SO_SNDTIMEO, buf, count);
}

size_t writev(int fd, const struct iovec *iov, int iovcnt) {
  return solar::do_io(fd, writev_f, "writev", solar::IOManager::WRITE,
                      SO_SNDTIMEO, iov, iovcnt);
}

ssize_t send(int s, const void *msg, size_t len, int flags) {
  return do_io(s, send_f, "send", solar::IOManager::WRITE, SO_SNDTIMEO, msg,
               len, flags);
}

ssize_t sendto(int s, const void *msg, size_t len, int flags,
               const struct sockaddr *to, socklen_t tolen) {
  return do_io(s, sendto_f, "sendto", solar::IOManager::WRITE, SO_SNDTIMEO, msg,
               len, flags, to, tolen);
}

ssize_t sendmsg(int s, const struct msghdr *msg, int flags) {
  return do_io(s, sendmsg_f, "sendmsg", solar::IOManager::WRITE, SO_SNDTIMEO,
               msg, flags);
}
