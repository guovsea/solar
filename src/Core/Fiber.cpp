...
void Fiber::MainFunc() {
    ...
    try {
        ...
        cur->m_state = EXCEPT;
        SOLAR_LOG_ERROR(g_logger) << "Fiber Except: " << ex.what() << " id = " << GetFiberID() << std::endl
                                  << BacktraceToString();
    } catch (...) {
        cur->m_state = EXCEPT;
    ...
    SOLAR_LOG_ERROR(g_logger) << " id = " << GetFiberID() << std::endl << BacktraceToString();
}
...
void Fiber::CallerMainFunc() {
    ...
    auto raw_ptr = cur.get();
    cur.reset();
    raw_ptr->back(); // 对于 UserCaller 的 Faber
