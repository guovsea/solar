 ...
 */
LogIniter::LogIniter() {
    auto updateLogSystem = [](const std::set<LoggerDefine> &oldVal, const std::set<LoggerDefine> &newVal) {
...
LogAppenderDefine LogAppenderDefine::FromYamlString(const std::string &str) {
    ...
    if (node["type"].IsDefined()) {
        ...
        if (typeStr == "FileLogAppender") {
            ...
            valid = false;
            std::cout << "log config error: log appender type is valid, " << node << std::endl;
        }
    } else {
        valid = false;
        std::cout << "log config error: log appender type is valid, " << node << std::endl;
...
LoggerDefine LoggerDefine::FromYamlString(const std::string &str) {
    ...
    if (node["appenders"].IsSequence()) {
        for (auto it = node["appenders"].begin(); it != node["appenders"].end(); ++it) {
