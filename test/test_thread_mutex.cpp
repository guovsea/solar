...
void work1() {
    ...
    SOLAR_LOG_INFO(g_logger) << "name = " << solar::Thread::GetName()
                             ...
                             << " id = " << solar::GetThreadId() << " this.id =" << solar::Thread::GetThis()->getId();
    // ps -p -T 查看线程信息
...
TEST(TestThreadMutex, TestThreadAndSemaphore) {
    std::vector<solar::Thread::ptr> thrs;
    for (int i = 0; i < 5; ++i) {
        solar::Thread::ptr pThr(new solar::Thread(work2, "worker_" + std::to_string(i)));
        thrs.push_back(pThr);
    }
...
TEST(TestThreadMutex, TestRwLock) {
    std::vector<solar::Thread::ptr> thrs;
    for (int i = 0; i < 5; ++i) {
        solar::Thread::ptr pThr(new solar::Thread(work3, "worker_" + std::to_string(i)));
        thrs.push_back(pThr);
    }
...
TEST(TestThreadMutex, TestRWScopedMutexLock) {
    std::vector<solar::Thread::ptr> thrs;
    for (int i = 0; i < 5; ++i) {
        solar::Thread::ptr pThr(new solar::Thread(work4, "worker_" + std::to_string(i)));
        thrs.push_back(pThr);
    }
...
TEST(TestThreadMutex, TestMutexLock) {
    std::vector<solar::Thread::ptr> thrs;
    for (int i = 0; i < 5; ++i) {
        solar::Thread::ptr pThr(new solar::Thread(work5, "worker_" + std::to_string(i)));
        thrs.push_back(pThr);
    }
...
TEST(TestThreadMutex, TestLoggerInMutiThread) {
    std::vector<solar::Thread::ptr> thrs;
    for (int i = 0; i < 5; ++i) {
        solar::Thread::ptr pThr(new solar::Thread(work6, "logger_thread_" + std::to_string(i)));
        thrs.push_back(pThr);
    }
